...
class LCSObject:
    ...
    def __init__(self, logTemplate='', logIDL=[]):
        ...
        ### template index in the bin, start from 0
...
class Node:
    ...
    def __init__(self, token='', templateNo=0):
        ...
        ### how many templates under this node
...
class LogParser:
    ...
    def SimpleLoopMatch(self, logClustL, seq):
        for logClust in logClustL:
            ...
            token_set = set(seq)
            if all(token in token_set or token == VAR for token in logClust.logTemplate):
                return logClust
        return None
        ...
        for i in range(idx, length):
            if seq[i] in parentn.childD:
                ...
                if (childn.logClust is not None):
                    constLM = [w for w in childn.logClust.logTemplate if w != VAR]
    ...
    def parse(self, logname):
        ...
        # rootNode = Node()
        logCluL = []
        ### element as tuple (rootNode, logCluL)
        bin_dict = {}
        eventId_list = []
        ...
        for idx, line in self.df_log.iterrows():
            ...
            if matchCluster is None:
                ...
                if matchCluster is None:
                    ...
                    if matchCluster is None:
                        ...
                        ### update templated_idx
                        templated_idx = newCluster.template_idx
            ...
            if matchCluster:
                ...
                ### update templated_idx
                templated_idx = matchCluster.template_idx

            ### save the eventId
            assert templated_idx is not None
            eventId_list.append(log_sig * 1000 + templated_idx)
    ...
    def get_parameter_list(self, row):
        template_regex = re.sub(r"\s<.{1,5}>\s", VAR, row["EventTemplate"])
